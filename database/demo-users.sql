-- Demo Users for BandhuConnect+ Testing
-- Create these users in Supabase Auth Dashboard first, then run this script
...
BEGIN
    ...
    IF user_id IS NOT NULL THEN
        ...
        VALUES (
            user_id, 
            'Amit Kumar (Hotel Manager)', 
            'amit.volunteer@demo.com', 
            '+919900000004', 
            'volunteer', 
            ARRAY['accommodation', 'general'], 
            'busy', 
...
BEGIN
    ...
    IF user_id IS NOT NULL THEN
        ...
        VALUES (
            user_id, 
            'Sneha Joshi (Tour Guide)', 
            'sneha.volunteer@demo.com', 
            '+919900000005', 
            'volunteer', 
            ARRAY['guidance', 'general', 'transportation'], 
            'offline', 
...
-- =============================================
-- CREATE DEMO REQUESTS AND ASSIGNMENTS
-- =============================================
...
BEGIN
    ...
    IF pilgrim_id IS NOT NULL THEN
        ...
        VALUES (
            pilgrim_id,
            'transportation',
            'Need ride from station to temple',
            'Elderly person with heavy luggage needs transportation from railway station to main temple. Difficulty walking long distances.',
            'medium',
            'pending',
            ST_SetSRID(ST_MakePoint(72.5720, 23.0280), 4326),
            'Ahmedabad Railway Station, Platform 1'
...
BEGIN
    ...
    IF pilgrim_id IS NOT NULL AND volunteer_id IS NOT NULL THEN
        ...
        VALUES (
            ...
            'My 8-year-old daughter is feeling dizzy and nauseous. Need basic medical assistance and advice.',
            'high',
            'assigned',
            ST_SetSRID(ST_MakePoint(72.5680, 23.0320), 4326),
            'Near Main Temple Gate, Rest Area'
        ) RETURNING id INTO request_id;
...
-- Demo Request 3: Food (In Progress)
DO $$
DECLARE
    pilgrim_id UUID;
    volunteer_id UUID;
    ...
    assignment_id UUID;
BEGIN
    SELECT id INTO pilgrim_id FROM auth.users WHERE email = 'mohan.pilgrim@demo.com';
    SELECT id INTO volunteer_id FROM auth.users WHERE email = 'amit.volunteer@demo.com';
    ...
    IF pilgrim_id IS NOT NULL AND volunteer_id IS NOT NULL THEN
        ...
        VALUES (
            pilgrim_id,
            'food',
            'Vegetarian meal for family',
            'Need simple vegetarian food for family of 3. Prefer traditional Gujarati thali. Budget friendly options.',
            'medium',
            'in_progress',
            ST_SetSRID(ST_MakePoint(72.5820, 23.0180), 4326),
            'Pilgrim Rest Area, Block B'
        ) RETURNING id INTO request_id;
        
        INSERT INTO assignments (request_id, volunteer_id, status, assigned_at, accepted_at, started_at)
        VALUES (
            request_id,
            volunteer_id,
            'in_progress',
        ...
        RAISE NOTICE 'Demo food request and assignment created: % -> %', request_id, assignment_id;
    END IF;
END $$;
...
-- Notifications for volunteers about new requests
DO $$
DECLARE
    raj_id UUID;
    priya_id UUID;
