...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(
      (
        ...
        FROM help_assignments ha
        JOIN help_requests hr ON ha.request_id = hr.id
        LEFT JOIN users pu ON hr.user_id = pu.id
        LEFT JOIN users vu ON ha.volunteer_id = vu.id
        WHERE ha.volunteer_id = p_volunteer_id 
        AND hr.user_id = ul.user_id
        AND ha.status IN ('assigned', 'in_progress')
      ),
      '[]'::jsonb
    ) as assignment_info
  ...
  AND EXISTS (
    SELECT 1 FROM help_assignments ha
    JOIN help_requests hr ON ha.request_id = hr.id
    WHERE ha.volunteer_id = p_volunteer_id
    AND hr.user_id = ul.user_id
    AND ha.status IN ('assigned', 'in_progress')
